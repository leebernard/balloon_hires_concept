...
if debug:
    ...
    test_ax.plot(wl, 1-raw_spectrum, label='Raw spectrum data', color='r', alpha=0.3)
    test_ax.set_xlabel('wavelength (um)')
    test_ax.set_ylabel('Fractional Transparency')
    test_ax.set_xlim(1.597, 1.602)
    test_ax.legend()
...
# N_lines_gem = np.sum(1-raw_spectrum[usable_spectrum])/3
